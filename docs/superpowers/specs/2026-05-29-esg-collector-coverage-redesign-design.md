...
- **task_id schemes** (globally unique, idempotent under `INSERT OR IGNORE`):
  ...
  - L2 alias: `{backend}:alias:{ticker}:{alias_ix}:{after}:{before}` — `before` is part of the id so a near-cap month and its weekly split-children (which share `after`) don't collide under `INSERT OR IGNORE` (review fix 2026-05-29).
...
- **One flow, two windows (`build_combined_tasks`, decided 2026-05-29).** Backfill and daily run the *same* builder — L2 alias + L1 keyword — differing only by window: backfill (`window=None`) uses each backend's historical settings window; `--mode daily` uses a recent window (default last 5 days, all backends, BaoMoi included — its early-stop keeps the pass to ~1–2 pages/alias). The legacy broad OR-keyword pool (`build_queue` / `KEYWORD_GROUPS`) is **retired** — the single-term L1 net supersedes it (no OR-clause limit). `--tickers` restricts alias/daily/backfill to specific tickers so a newly-added company is backfilled once without re-running the whole universe (idempotent).
