...
- **Short-form derivation (alias quality):** the page gives full legal names ("Công ty TNHH Thức ăn chăn nuôi Nasaco Hà Nam"). Store the full name as a (safe, specific) alias, and derive a distinctive short token ("Nasaco") by stripping legal prefixes (`Công ty TNHH/CTCP/MTV/Tổng Công ty`, business-type words). **Skip short tokens that are too generic/common** (e.g. bare "Minh Phát", "Song Lập") to avoid cross-company false positives — when unsure, keep only the full name. Subsidiaries whose name already contains the parent brand ("Dabaco Thanh Hóa") are redundant with the parent alias but harmless.
